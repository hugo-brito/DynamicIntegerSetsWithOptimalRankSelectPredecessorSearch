...
In order to analyze and describe running times, computer scientists use models of computation. Such a model states which operations have an associated cost, and which ones do not.
...
The models are presented in descending order from strongest to the least strong. This means that a less restrictive model, such as the cell-probe model, is more suited to describe the theoretical \emph{lower} bounds of a data structure than the weaker ones \cite{erikdemainelec11}.
...
\begin{align*}
    ...
    \implies &16 \geq n
\end{align*}
We can see that if the word length, $w$, is $4$, then we can at most index $16$ keys. This is sound because in binary representation of integers, when having $4$ bits, we can produce $16$ different combinations of those bits, e.g., $2^4$ combinations. This is exactly what expression~\ref{eq:problemSize} means: having a set word length defines the maximum number of keys we can index with a single word.
...
If, as discussed in Section~\ref{sec:word}, $u = 2^w$ is the size of universe of keys we can store in this data structure, then we can also express the running time as being $O(\log_2(\log_2 u))$.
...
This data structure is defined recursively, and its running time given by the recurrence relation: $\text{T}(u) = \text{T}(\sqrt{u}) + O(1)$.
A vEB tree stores 

nice and summary slides from DTU: http://www2.compute.dtu.dk/courses/02282/2016/predecessor/predecessor1x1.pdf
...
To compute the parent nodes, we take the bit-wise $\vee$ of each consecutive pair of positions.
...
This rule applies also to each of the sub-trees.
On this preliminary example:
\begin{itemize}
    \item
    To find the minimum, starting at the root, we traverse the tree by taking always the left child, if that node is $1$, otherwise we take the right child.
    
    \item
    To find the maximum, we do the same as in finding the minimum, but giving preference to the right child.
    
    \item
    To find the successor, we would start at the leaf node of our query and go up the tree until we find a node whose right child we have not visited and whose value is $1$. The successor of our query is the minimum of the sub-tree rooted at that node.
    
    \item
    Finding the predecessor will be similar to the successor: we find the maximum of the sub-tree whose root's left child we have not visited and has value $1$, when going up the tree from the query's leaf node.
...
To improve this and achieve the sub-logarithmic running times, we superimpose a tree of degree of varying degree on the bit-vector. The value of root node will be the summary of the whole range. Its children will cover $u^{1/2}$, and its grandchildren $u^{1/4}$. This approach gives us the following recurrence relation: $\text{T}(u) = \text{T}(\sqrt{u}) + O(1)$.

The van Emde Boas tree is defined recursively, and each node contains the following:
\begin{itemize}
    \item
    Its minimum.
...
look at the bit values in the bit-vector.
The values of the nodes of the tree imposed on the bit-vector will be the bit-wise 
...
\begin{enumerate}
    ...
    \item
    We \textit{compress} the summary such that it fits in a single $\sqrt{w}$-bit field. A bit-wise shift operation is also carried in order to compute a word containing the resulting summary on its least significant field.
...
\chapter{Implementation of theoretically optimal dynamic rank, select, and predecessor data structure} \label{sec:implementationsChapter}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item
        We compute $branch \vee (\hat x^k \wedge free)$, which is done by bit-wise $\vee$ tables~\ref{tab:branchTable} and \ref{tab:xCompressedANDfree}. This operation will use the actual bits of $\hat x^k$ in the "don't cares" positions of all the compressed keys with "don't cares" in the set. Thus we end up with:
        \begin{table}[H]
        \centering
...
The {\ttfamily insert(final long x)} is now implemented the following way \cite{patrascu2014dynamic}:
\begin{enumerate}
    \item
...
Apart from step \ref{blt:naiveRankJ}, all the steps taken by this algorithm run in $O(1)$ time.
...
\begin{enumerate}
    \item
    Varying $f$ (being $f$ the size of each field) between $1$ and $w - 1$.
