...
The result is a Trie with exactly as many nodes as there are keys, which also improves the worst running times of updating and querying from $O(w)$ to $O(\log_2 n)$ when compared to the Binary Search Trie \cite{sedgewick2002algorithms}.
...
It uses the fact that the keys themselves can be seen as memory addresses to do so \cite{nelsonjelanilec1}.
...
This is due to the bit-vectors used, which in total allocate space for all the possible keys in the universe.
...
This means that, without any additional tools and algorithms, we cannot store the bits of every key in a single word so we can process each node in $O(1)$ time.

Using only algorithms based on word RAM operations, listed in Section~\ref{sec:wordRAM}, we can query a Fusion Node with $O(1)$ number of operations.
...
Upon its construction, each node will contain $k$ keys.

We note that, for any given node, when looking at the keys' values in binary and building a Bitwise Trie with the keys at that node, we need only to know at most $k - 1$ bit indices to differentiate the keys in that node.
These are the branching bits in the Bitwise Trie.
...
The key difference between the \textit{perfect sketches} and \textit{approximate sketches} is the amount of bits used to store the \textit{sketches} of the keys: \textit{approximate sketches} use at most $(k - 1)^4 = O(w^{4/5})$ instead of the  $O(w^{2/5})$ of the \textit{perfect sketches}.
...
Computing $\text{sketch}(q)$ will give us other two keys in the node, $x_i$ and $x_{i+1}$, where this sketch lies between.
...
It is important to note that \cite{patrascu2014dynamic} mentions only that $k$ has to be a power of $2$. The implementation allows for $k$ to take other values just as long as they are at most $16$ and a power of $2$.
...
Validation summary tables, which include the results of the test, together with the range or size of the test instances, have been included.
