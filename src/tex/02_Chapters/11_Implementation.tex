...
In this incremental step, we keep the most of the implementation details of \ref{sec:DynamicFusionNodeBinaryRank}, plus we store the compressed keys with "don't cares" to enable the rank operation as described in the \textit{matching with "don't cares"} chapter of \cite{patrascu2014dynamic}.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        When seeing the keys in a matrix and ordered by their rank, we can see that the value of some of those positions has no influence in the rank of the (compressed) keys.
    ...
    Now assume that we have a word, where each of that word's fields stores a compressed key. If we interpret this word as a matrix, then its rows (which are simply the word's fields) will correspond to a compressed key, and its columns will correspond to the indices of bits in the compressed keys, e.g., significant positions.

    \item
    A subroutine $match(x)$ that uses all of the above and rank via Rank Lemma 1 (defined in Section~\ref{sec:rankLemma1}).
    
    Let $\hat x^k$ be the compressed version of $x$ copied $k$ times and stored in a word. We use the $M$ helper function from the {\ttfamily Util} class to achieve this. Note that we have also done similar in \ref{sec:computeM}.
...
private long compressingKey;
    ...
    Each column $j$, e.g., $branch\langle i, j\rangle_{1 \times k}$ corresponds to a branching bit.
    \begin{lstlisting}
private long branch;
    \end{lstlisting}
...
private long dontCares(long compressedKeys, long free, final int bit, final int lo, final int hi)
    \end{lstlisting}
    The parameters $compressedKeys$ and $free$ are the compressed keys stored in a word and the data about "don't cares", respectively. $bit$ refers to index of the column (in both $compressedKeys$ and $free$), whereas $hi$ and $lo$ refers to a range of rows (also in both $compressedKeys$ and $free$).
    It follows these steps to update $free$:
    \begin{enumerate}
        \item
...
updateBranch(compressedKeys);
    ...
    \item
    {\ttfamily reset()}.
    Resetting the data structure now also entails resetting some of the class variables. The additional operations are:
    \begin{itemize}
        \item
...
This section is aimed at showcasing how the compressed keys with "don't cares" are stored in the data structure and what are the steps taken by the algorithm to return a {\ttfamily rank(x)} query.

We will have a concrete example, which consists of the keys from table~\ref{tab:keysInTheSetForRank}. For brevity, the selected set of keys will only have bits set between the indices 0 and 11. This way, we know that the 52 leading bits will have no influence when running the algorithm.
...
Our rank query will be with the key $x = 1010\ 1010\ 1111_2$.
...
In the data structure, this data is stored in the two class variables $branch$ and $free$. They are each a single word. If we lay them in a matrix, then each row will correspond to a compressed key.

\begin{table}[H]
\centering
\input{04_Tables/007_branchTable.tex}
\caption{The resulting word $branch$ laid in a $k \times k$ matrix}
\label{tab:branchTable}
\end{table}
...
Sections \ref{sec:keyCompression} and \ref{sec:keyCompressionWithDontCares} have handled the contents of the set and the important class variables for this operation. Now we shall now see what unfolds as soon as we query $\text{rank}(1111\ 1001\ 0010_2)$.
...
\begin{enumerate}
    ...
    \item
    A call to {\ttfamily match} with the query $x$ is done. In $\text{match}(1111\ 1001\ 0010_2)$. The operation entails compressing the query, computing the word $branch \vee (\hat x^k \wedge free)$ and computing the rank of $\hat x$ in that word via Rank Lemma 1.
    \begin{enumerate}
        \item
        We compress the query with $\text{compress}(1111\ 1001\ 0010_2)$. Thus we have:
        \begin{align*}
            compressingKey = 1111\ 1001\ 0010_2\\
        ...
        \item
        We compute $\hat x^k \wedge free$, which keeps only the "don't cares" bits in $\hat x^k$. By ANDing tables \ref{tab:xCompressedCopied} and \ref{tab:freeTable} we have:
        \begin{table}[H]
        \centering
        ...
        \item
        We compute $branch \vee (\hat x^k \wedge free)$. This will project the bits of $\hat x^k$ in the "don't cares" positions of all the compressed keys in the set. Thus we end up with:
        \begin{table}[H]
        \centering
    ...
    \item
    We perform the query $select(i)$:
    \begin{align*}
        i = 5\\
        \cline{1-2}
        select(5) = 1010\ 1100\ 1111_2
    \end{align*}
    ...
    \begin{align*}
        ...
        x < y \implies comp = -1
    \end{align*}
    If $comp = 0$ then $x = y$, which means that $x$ is already present in the set, thus its rank will be $i$. In this case, the method returns $i$. This is not the case; thus, we proceed.
    ...
    To find the actual rank of $x$, we need to use the match function once again. We know now that there is a key, $y$, that matched $x$ that branched away from $x$ in the $6^{th}$ bit. The match of $x$ is either a successor or a predecessor of $x$.
    We have now two cases:
    \begin{itemize}
        \item
        If $comp < 0$ then $x < y$ and $y$ is a successor of $x$, so we apply a mask to $x$ that keeps only its $w-j$ most significant bits. Let the value of $x$ after this mask be $x_m$; when applying \textit{match} to $x_m$, we go down the tree to the left as much as possible after the branching bit $j$ because all the bits of $x_m$ after $j$ will be zero. The rank of $x$ will be the $match(x_m)$, which Pătrașcu and Thorup denote by $i_0$ in \cite{patrascu2014dynamic}.
        
        \item
        Otherwise, $x > y$ and $y$ is a predecessor of $x$. We apply a mask to $x$ that set its $j$ least significant bits to one. When applying \textit{match} to $x$ with this mask we go down the tree to the right as much as possible after the branching bit $j$ because all the bits of $x_m$ after $j$ will be one. The rank of $x$ will be the $match(x_m) + 1$, where, second Pătrașcu and Thorup, $match(x_m) = i_1$ in \cite{patrascu2014dynamic}.
    \end{itemize}
    ...
    \begin{align*}
        ...
        x_m = \underline{1010}\ \underline{10}10\ 1111_2\\
        \cline{1-2}
        x_m = x \wedge mask = \underline{1010}\ \underline{10}00\ 0000_2
    \end{align*}

    Finally we call {\ttfamily match} with $x_m$:
    \begin{enumerate}
        \item
        We compress the query with $\text{compress}(1111\ 1000\ 0000_2)$. Thus we have:
        \begin{align*}
            compressingKey = 1111\ 1001\ 0010_2\\
            \hat x_m = \underline{1010}\ \underline{1}00\underline{0}\ 00\underline{0}0_2\\
            \cline{1-2}
        ...
        We compute $\hat x^k_m \wedge free$, keeps only the "don't cares" bits in $\hat x^k$. By ANDing tables \ref{tab:xMaskedCompressedCopied} and \ref{tab:freeTable} we have:
        \begin{table}[H]
        \centering
        ...
        \item
        We compute $branch \vee (\hat x^k_m \wedge free)$. This will project the bits of $\hat x^k_m$ in the "don't cares" positions of all the compressed keys in the set. Thus we end up with:
        \begin{table}[H]
        \centering
...
\begin{itemize}
    ...
    \item
    Computing the rank of new significant positions. In other words, if adding a new key implies adding a new column $j$ in $branch$ and $free$ because it is a new significant position, then we need to know how many set bits are there in the $compressingKey$ up to $j$.
    
    \item
    Matrix operations including in $branch$ and $free$ such as:
    \begin{itemize}
        \item
...
All the class variables remain unchanged in comparison with \ref{sec:dontCaresInsertFields}. They are:
\begin{lstlisting}
private static final int k = 8;
private static final int ceilLgK = (int) Math.ceil(Math.log10(k) / Math.log10(2));
private static final long M = Util.M(k, k * k);
...
\begin{itemize}
    ...
    \begin{table}[H]
    \centering
    \input{src/tex/04_Tables/019_MatrixRowRange.tex}
    ...
    \end{table}
    A call to {\ttfamily insertAndInitializeColumn(2)} will move all columns larger than $2$ one position to the left and set values in the new column them to their respective default. In \ref{tab:branchAndFreeAfterColumnInsertion} we can see the new column highlighted.
    \begin{table}[H]
    \centering
    ...
    \item
    {\ttfamily insertRow(final int rank)} introduces a new row in $branch$ and $free$ at position $rank$. All rows with rank larger than $rank$ will occupy their respective next row. Applying {\ttfamily insertRow(2)} on $branch$ and $free$ from \ref{tab:branchAndFreeExample} will result on the following:
    \begin{table}[H]
    \centering
...
\begin{enumerate}
    \item
    A placeholder for a local variable $rank$ is allocated.
    
    \item
    If the set is not empty:
    \begin{enumerate}
        \item
        We compute $match(x)$ and store it in a local variable $i$.
        
        \item
        We query $select(i)$ and and store it in a local variable $y$.
        
        \item
        We use the unsigned comparator from the standard library {\ttfamily compareUnsigned} to assess if the keys, $x$ and $y$, are equal of one if larger than the other. This is stored in a local variable $comp$.
        ...
        \item
        We compute $i_0$ and $i_1$, given by the calls $match(x \wedge \neg((1 \ll j) - 1))$ and $match(x \vee ((1 \ll j) - 1))$. These masks and queries have been explained in \ref{sec:rankDontCaresQuery} and they will correspond to rows in $branch$ and $free$, e.g. concrete keys in the set.
        ...
        \begin{itemize}
            ...
            \item
            Update $branch$ and $free$ with a call to {\ttfamily insertAndInitializeColumn(h)}.
        \end{itemize}
        
        \item \label{blt:matrixRowColumnMask}
        We call {\ttfamily matrixMRowRange(i\_0, i\_1)} and intersect it with  {\ttfamily matrixM(h)}. This will produce a mask that has column $h$ in the range of rows between $i_0$ and $i_1$ set to one.
        ...
        \item
        We insert a new row in $branch$ and $free$ with a call to {\ttfamily insertRow(final int rank)}
        
        \item
        The local variable $i$, stores the rank of $y$, is updated. This means that if $x < y$, then the rank of $y$ has incremented.
